...
$db=new dbHelper();
		...
		$sql="select *from comnews where id=1";
		$res=$db->execute_select($sql);
		$fenlei=explode('|', $res[0][7]);
		$nums=count($fenlei);
if($_GET['fenlei']!=""){
	$ff=$_GET['fenlei'];
	$sql="select *from comnews where leibie='$ff'";
	$res=$db->execute_select($sql);
}else
	{
		$sql="select *from comnews where id>1";
...
<head>
	...
	<script type="text/javascript">
			function update(i){
				...
				window.location.href='../option/updateNew.php?id='+i+'&d='+'comnews'+'';
			}
			function changfenlei(){
				window.location.href='comnews.php?fenlei='+document.getElementById("fenlei").value+'';
			}
			function deletenew(id){
				window.location.href='../option/deletefile.php?id='+id+'&table='+'comnews'+'';
			}
	</script>
</head>
									...
									<table class="table">
										...
										<thead>
											<tr class="name1">
												...
												<th><input type="button" name="addfenlei" class="btn btn-info" value="添加分类" onclick="window.location.href='../option/addfenlei.php?page=comnews'">
												<input type="button" name="add" class="btn btn-info" value="添加文章" style="margin-left: 0px;" onclick="window.location.href='../option/addnews.php?page=comnews'">
												</form>
											</tr>
										</thead>
										<thead>
